...
export default function MoviePlayer({
  ...
  useEffect(() => {
  const fetchRelated = async () => {
    if (!movie?.genres || !movie?._id) return;
    const { next, prev } = await getNextPrevMediaByGenre("movie", movie.genres, movie._id);
    setNextId(next);
    setPrevId(prev);
  };
  fetchRelated();
  ...
  return (
    <div className="relative w-full h-screen bg-black text-white">
      <CustomVideoPlayer
        ...
        type={type}
        media={movie}  
        nextEpisodeId={nextId}  // we pass nextId as nextEpisodeId for consistency
        onNextEpisode={(nextId) => {
          if (nextId) {
            window.location.href = `/watch/movie/${nextId}`;
          }
        }}
      />
    </div>
