...
export default function EpisodePlayer({
  ...
  // Normalize HLS URL source for episode
  const episodeHLS = typeof episode.HLS === "string" ? { master: episode.HLS } : episode.HLS || {};
  const sourceUrl = episodeHLS.master || episode.transcodedVideo || episode.video_url_s3 || episode.trailerUrl;
  ...
  return (
    <div className="relative w-full h-screen bg-black text-white">
      <CustomVideoPlayer
        ...
        mediaId={mediaId}
        media={episode} 
        parentTVShow={episode.parentTVShow}  // ✅ We pass the parentTVShow info here
        ...
        onNextEpisode={(nextId) => {
          if (nextId) {
            router.push(`/watch/episode/${nextId}`);
          }
        }}
      />
    </div>
